...
export class Gpu extends Component<backend.IdcProps> {
    ...
    componentDidMount() {
        this.tdpInterval = setInterval(() => {
            if (this.state.selectedTDPValue !== null) {
                set_value(TDP, this.state.selectedTDPValue);
            }
        }, 3 * 60 * 1000);
    }

    componentWillUnmount() {
        clearInterval(this.tdpInterval);
    }
    ...
    render() {
        ...
        const performanceDropdown: SingleDropdownOption[] = [
            ...
            {data: 3, label: <span>Overdrive 40W</span>}, 
            {data: 4, label: <span>Manual</span>}
        ];

        const labels : string [] = ["Silent 10W", "Performance 15W", "Turbo 25W", "Overdrive 40W", "Manual", "Performance 20W"];
        ...
        return (<Fragment>
            ...
            this.setState({ selectedTDPValue: (get_value(LIMITS_INFO) as backend.SettingsLimits).gpu.slow_ppt_limits!.max / 2000 });
                            }
                            ...
                            set_value(TDP, null);
            this.setState({ selectedTDPValue: null });
            ...
            this.setState({ selectedTDPValue: limits[0]/1000 });
            ...
            this.setState({ selectedTDPValue: limits[0]/1000 });
                                    ...
                                    break;
                                case 3:
                                    backend.resolve(backend.setGpuPptTdp(40000, 53000, 45000), (limits: number[]) => {
                                        set_value(TDP, limits[0]/1000);
            this.setState({ selectedTDPValue: limits[0]/1000 });
                                        set_value(FAST_PPT_GPU, limits[1]);
                                        set_value(SLOW_PPT_GPU, limits[2]);
